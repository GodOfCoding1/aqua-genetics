Shader "Aquarium/FishPattern"
{
    ...
    SubShader
    {
        ...
        float4 frag(Varyings i) : SV_Target
        {
            ...
            // Countershading: belly (uv.y near 0) is lighter, dorsal (uv.y
            // near 1) is darker. Real fish exhibit this so they're harder to
            // spot from above and below; reading it on a 2D side-view
            // dramatically improves the "this is a fish" silhouette.
            float belly = 1.0 - i.uv.y;
            float shade = lerp(0.78, 1.18, smoothstep(0.0, 1.0, belly));
            col *= shade;

            // Inner-edge darkening using the boundary distance carried in uv2.x
            // (0 = body centre vertex, 1 = silhouette vertex). Pixels within
            // ~8% of the silhouette get a soft dark band so the body reads as
            // a rounded volume rather than a flat shape.
            ...
            // Transparency gene: simulate as fade (opaque draw so 2D URP always shows the body).
